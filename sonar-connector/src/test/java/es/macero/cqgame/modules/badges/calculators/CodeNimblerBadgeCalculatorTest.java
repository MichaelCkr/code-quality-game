...
public class CodeNimblerBadgeCalculatorTest {
	...
	private CodeNimblerBadgeCalculator calculator = new CodeNimblerBadgeCalculator();
	...
	public void testFixedIssuesFirstThresholdNoBadge() {
		...
		assertThat(result.orElse(FAILURE).getName(), is("Coordinated Coder"));
	}
	...
	public void testFixedIssuesSecondThresholdNoBadge() {
		...
		assertThat(result.isPresent(), is(true));
		assertThat(result.orElse(FAILURE).getName(), is("Agile Coder"));
	}
	...
	public void testFixedIssuesThirdThresholdNoBadge() {
		...
		assertThat(result.isPresent(), is(true));
		assertThat(result.orElse(FAILURE).getName(), is("Dexterous Coder"));
	}
	...
	public void testFixedIssuesFourthThresholdNoBadge() {
		...
		assertThat(result.isPresent(), is(true));
		assertThat(result.orElse(FAILURE).getName(), is("Graceful Tumbling Expert"));
	}
